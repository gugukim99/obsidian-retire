# =============================================================================
# fetch_news.ps1 - 10분마다 웹 뉴스 수집 -> .md 파일 생성 (PowerShell)
...
Write-Log "=== 뉴스 수집기를 시작합니다 ==="
Write-Log "수집 간격: ${Interval}초 (10분)"
Write-Log "저장 위치: $NewsDir"
...
"@
    ...
    try {
        ...
        if (Test-Path $OutputPath) {
            ...
            Write-Log "[#${Count}] 뉴스 저장 완료: $Filename ($FileSize bytes)"
        }
        else {
            Write-Log "[#${Count}] WARNING: Claude가 직접 저장하지 못했습니다. stdout 리다이렉트로 재시도..."
...
"@
            ...
            if ($result) {
                ...
                $FileSize = (Get-Item $OutputPath).Length
                Write-Log "[#${Count}] 대체 방법으로 저장 완료: $Filename ($FileSize bytes)"
            }
            else {
                Write-Log "[#${Count}] ERROR: 뉴스 수집에 실패했습니다."
            }
        }
    }
    catch {
        Write-Log "[#${Count}] ERROR: $_"
    }

    Write-Log "[#${Count}] 다음 수집까지 10분 대기..."
